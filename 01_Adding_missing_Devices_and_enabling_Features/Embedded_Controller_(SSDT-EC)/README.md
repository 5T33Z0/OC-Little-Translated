...
**SSDTTime** is a python script which can generate various SSDTs by analyzing your system's `DSDT`. Unfortunately, SSDT-EC tables generated by SSDTTime **do not include the `USBX` Device**. So if you are using a system with a Skylake or newer CPU, you either have to add an addtional SSDT containing the USBX device or add it the to the existing SSDT-EC or use the manual method instead!
