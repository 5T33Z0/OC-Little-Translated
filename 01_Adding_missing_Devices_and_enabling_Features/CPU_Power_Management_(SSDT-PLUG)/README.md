...
7. Include the newly added SSDT(s) to your config's ACPI > Add Section. If you are editng your config using [**OpenCore Auxiliary Tools**](https://github.com/ic005k/QtOpenCoreConfig/releases) (OCAT) it will add them to the list automatically, since it monitors the ACPI folder.
8. Save your config.plist and reboot

After the restart, enter in Terminal: `sysctl machdep.xcpm.mode`</br>
If the output is `1`, it's working. If it's `0` it's not.
