...
Enabling ACPI debugging is relatively simple: grab `SSDT-RMDT.aml` (RehabMan Debugging Table) and `ACPIDebug.kext` from the [ACPI Debuggig Section](https://github.com/5T33Z0/OC-Little-Translated/tree/main/00_ACPI/ACPI_Debugging)) and add them to your EFI and config.plist. The (relatively) difficult part is writing an SSDT for debugging.
