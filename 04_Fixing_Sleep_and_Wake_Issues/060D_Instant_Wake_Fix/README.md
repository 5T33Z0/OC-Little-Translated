...
3. Add a binary rule to `ACPI/Patch`, depending on the method used in your `DSDT`:
   * Rename `GPRW to XPRW` or
   * Rename `UPRW to XPRW` (see [**`SSDT-XPRW.dsl`**](i\_Common\_060D\_Patch/SSDT-XPRW.dsl) for instructions).
   * :bulb: You may want to limit its reach by specifying a CPI path under `base`, like `_SB_.PCI0` for example.
4. Open `SSDT-XPRW.dsl` (located in the "i\_Common\_060D\_Patch" folder) in maciASL
5.  Add the APCI paths of devices which require `0D/6D` patches and add them as "External" references, for example:
    ...
    ```
6. Export the file as `SSDT-XPRW.aml`, add it to the `EFI/OC/ACPI` folder and your `config.plist`.
7. Save and reboot.
...
*   `Method type` of two: `Scope`
    ...
    This is not a common case. For the example case, using the binary rename _**Name6D-03 to 00**_ will work. Try other forms of content on your own.
...
There have been [reports](https://www.reddit.com/r/hackintosh/comments/7hl68w/modified\_dsdt\_cleared\_out\_all\_pwr\_entries\_sleep/) that removing the `_PRW` method from the `DSDT`completely solves this issue.
...
* `_PWR` (PowerResource for Wake) [Specifications](https://uefi.org/specs/ACPI/6.4/07\_Power\_and\_Performance\_Mgmt/device-power-management-objects.html#prw-power-resources-for-wake)
