...
## Essentials
The essential tools nearly every Hackintosh build will need, regardless of hardware. If you use **OCAT**, it already covers EFI mounting, SMBIOS generation updating OpenCore and kexst as well as config validation — so the tools marked *(not needed if using OCAT)* are only relevant if you go the **ProperTree** route instead.
...
| [**Hackintool**](https://github.com/headkaze/Hackintool) | All-purpose post-install diagnostics and patching. |
| [**GenSMBIOS**](https://github.com/corpnewt/GenSMBIOS) | Generate valid SMBIOS/serial data. *(Not needed if using OCAT.)* |
| [**SSDTTime**](https://github.com/corpnewt/SSDTTime) | Dump DSDT and generate common ACPI hotpatches. |
| [**MaciASL**](https://github.com/acidanthera/MaciASL) | Compile/edit ACPI tables (ASL/AML). |
| [**USBToolbox**](https://github.com/USBToolBox/tool) | Map USB ports correctly. |
| [**ocvalidate**](https://github.com/acidanthera/OpenCorePkg) (bundled in `Utilities/ocvalidate`) | Validate your `config.plist` before boot. *(Not needed if using OCAT.)* |
...
### Getting macOS
*Tools for downloading macOS directly from Apple Servers as well as creating installation media. Requires macOS to be installed already!*

- [**Download Full Installer**](https://github.com/perez987/DownloadFullInstaller) – Downloads Legacy OSX as well ass current macOS Installers (Big Sur+) for Intel and Apple Silicon from different Seeds (Regular, Customer, Developer, Public).
...
- [**MIST**](https://github.com/ninxsoft/Mist) (macOS Installer Super Tool) – Can download ipsw Firmwares (ARM) as well as macOS Installers for Intel-based systems (OSX 10.7.5 and newer). Check the "Settings…" menu to select the diplayed seed(s) of the Software Update Catalog, etc. Requires macOS Monterey or newer.
...
- [**TINU**](https://github.com/ITzTravelInTime/TINU) – Open Source Tool to create bootable macOS installers.
...
### EFI / config.plist Editing & Update Tools
...
### Port & Peripheral Mapping
- [**USBToolbox**](https://github.com/USBToolBox/tool) – USB Port mapping tool for Windows. Makes port mapping for macOS 11.3+ easier.
- [**GenI2C_Refresh**](https://github.com/Baio1977/GenI2C) – Updated tool for generating SSDT hotpatches for I2C controlled TouchPads for use with the VoodooI2C.kext. *(laptop-specific)*
...
- [**Plist Viewer & Editor**](https://plist-viewer.com/) – Online Plist Editor. Processes files locally in-browser, no uploads.
- [**OpenCore ScanPolicy Generator**](https://oc-scanpolicy.vercel.app/) – Generates the `ScanPolicy` hex value by ticking the file systems/device types you want OpenCore to scan.
- [**OpenCore ExposeSensitiveData Generator**](https://dreamwhite-oc-esd.vercel.app/) – Generates the `ExposeSensitiveData` hex value for controlling what OpenCore exposes as UEFI variables.
- [**Big <> Little Endian Converter**](https://www.save-editor.com/tools/wse_hex.html) – Converts values between hex, decimal, big-endian and little-endian byte order.
- [**Base64 Decoder/Encoder**](https://www.base64decode.org/) – Encodes/decodes Base64 strings and files.
...
## Further Resources
- [**OpenCore Install Guide**](https://dortania.github.io/OpenCore-Install-Guide/) by Dortania
