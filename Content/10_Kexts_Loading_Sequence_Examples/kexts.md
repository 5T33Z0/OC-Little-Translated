...
|                             | [**iBridged**](https://github.com/Carnations-Botanica/iBridged) | Patches Apple’s `BridgeOSInstall` framework to inject the `apple-coprocessor-version` variable, simulating a T2 coprocessor. This enables OTA updates on T2 SMBIOSes without requiring `board-id-skip` or `vmm-board-id` workarounds, while keeping `SIP` and `SecureBootModel` enabled. Major benefit: delta updates no longer fail – unlike with RestrictEvents, where they fail the first time and then the entire OS is downloaded afterwards. Unfortunately, using iBridged is only practical when *not* spoofing the vmm board-id. For macOS Tahoe, this limits the range of available SMBIOSes to iMac20,x, MacPro7,1, and MacBookPro16,1/2/4.
