...
- [Utilizing `MinKernel` and `MaxKernel` settings](#utilizing-minkernel-and-maxkernel-settings)
...
Applying `MinKernel` and `MaxKernel` settings for kexts is very useful to maximize the compatibility of your `config.plist` with *various* versions of macOS without having to create multiple configs with different sets of kexts. 

By specifying the kernel range, you can take control over which kexts are loaded for which version of macOS. Instead of enabling/disabling kexts manually for different OSes, you can leave all kexts enabled but control which ones will be loaded by entering values in the `MinKernel` and `MaxKernel` fields. "Example 7" and "Example 8" makes use of this technique extensively.

It's basically the same feature Clover provides, just a lot smarter: instead of using sub-folders labeled by the macOS Version (10.15, 11, 12, etc.), you specify the lower and upper kernel limit, so that you don't have to create kext duplicated (which you maybe forget to update later). 

**`MinKernel` and `MaxKernel` settings sare especially useful for**:

- **WiFi** and **Bluetooth** kexts where certain macOS versions require different sets of kexts
- Kexts which are only required by certain versions of macOS, like `CryptexFixup.kext` (Ventura and newer only) or `NoTouchID.kext` (High Sierra to Mojave only)

### Kernel Support Table

The following table contains the Kernel version ranges for OSX 10.4 up to macOS 15. 
...
macOS 11 (Big Sur)         | 20.0.0    | 20.99.99 | "
||
macOS 10.15 (Catalina)     | 19.0.0    | 19.99.99 |Intel (64-bit only)
||
macOS 10.14 (Mojave)       | 18.0.0    | 18.99.99 | Intel (32/64-bit)
...
OS X 10.6 (Snow Leopard)   | 10.0.0    | 10.99.99 | "
||
...
> [!TIP]
>
> - To check, which Kernel your macOS version is using, either enter `uname -r` in Terminal or look it up in "System Profiler" under "Software". 
> - Although `MaxKernel` can go up to `X.99.99`, using `X.9.9` is sufficient in most cases. So far, there hasn't been a single version of macOS which used a Kernel greater than `X.9.9`.
...
7. Next, add `MinKernel` and `MaxKernel` settings to limit the kext to only load the kext for the macOS version it’s designed for:<br>![itlwm](https://github.com/user-attachments/assets/c505f572-e8ca-4f21-bf84-c42b8a9a7c4f)
8. Save your config
...
> - The pink combination requires additional configuration (&rarr; See [Enabling AirportItlwm in macOS Sequoia/Tahoe](https://github.com/5T33Z0/OCLP4Hackintosh/blob/main/Enable_Features/AirportItllwm_Sequoia.md) for instructions).
...
- Thanks to Acquarius13 for explaining how to enable te Wifi Root Patches in OCLP!
