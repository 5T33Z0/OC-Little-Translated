...
- **Optional**: If the patches can't be applied due to config restrictions, lift the restrictions by adjusting the mentioned setting(s) in the config.plist. In this eaxample, `SecuredBootModel`has to be set to `Disabled` in the config.plist first before root patches can be applied.
